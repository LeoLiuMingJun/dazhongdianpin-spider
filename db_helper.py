...
class DBHelper:
    ...
    def __int__(self):
        self.server = SSHTunnelForwarder(ssh_address_or_host=('182.254.162.215', 22),  # 指定SSH中间登录地址和端口号
                                         ssh_username='root',  # 指定地址B的SSH登录用户名
                                         ssh_password='`Id310106199409200014',  # 指定地址B的SSH登录密码
                                         local_bind_address=('127.0.0.1', 3306),
                                         # 绑定本地地址A（默认127.0.0.1）及与B相通的端口（根据网络策略配置，若端口全放，则此行无需配置，使用默认即可）
        ...
        self.server.start()
        self.db = connect(host="127.0.0.1", port=self.server.local_bind_port, user="root", passwd="1qaz2wsx", db="dzdp")
    ...
    def select_data(self, sql):  # 由于自身习惯擅长处理list类型 所有返回list类型
